#锁 aqs AbstractQueueSynchronizer 同步器的运行机制
...
当线程获取锁失败则 线程和当前的等待状态信息构建Node对象放入同步阻塞队列尾部 阻塞等待再次获取锁。
 ...
 同步队列 双向链表 Node head node Tail 链表尾部插入 条件队列 双向链表 Node firstWorker LastNode

waitStatus 属性的值 0普通同步节点 -2 条件节点. await 释放锁 进入条件队列阻塞等待。
...
移动到同步队列唤醒节点中的线程。 实际上就是把 等待队列中的节点转移到同步队列中。
